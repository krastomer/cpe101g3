...
//                Log.d("MainActivity",boardid)
                for(h in p0.children){
                    ...
                    if(h.child("uid").value.toString()==boardid) {
                        Log.e("Calling",(LocalTime.now().hour.toString().toDouble()-2).toString())
...
//                        }
                ...
                view.findViewById<TextView>(R.id.totalmonth).text = "Monthly Static :   Total "+totalmonth.toString()
                view.findViewById<TextView>(R.id.totalyear).text = "Yearly Static :   Total "+totalyear.toString()
